...
class TokenAmount:
    ...
    def __str__(self): #позволяет выводить значение. пример для кода ниже: print(balance), т.е. когда не используется balance.Ether и balance.Wei (но такой сценарий маловериятный имхо)
        return f'{self.Ether}'
